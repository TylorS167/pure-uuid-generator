...
export function createWithSeed(seed: number): UuidGenerator {
  ...
  const clockSeqHi = 0x80 | getRandomInteger(timeHi, 0, LIMIT_UI06)
  const clockSeqLow = getRandomInteger(clockSeqHi, 0, LIMIT_UI08)
  const node = getRandomInteger(clockSeqLow, 0, LIMIT_UI48)
